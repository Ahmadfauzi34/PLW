#!/usr/bin/env python3
"""Prepare one sealed Prettier boolean-guard experiment before mutation.

This helper is candidate-specific and reference-only. It runs the exact focused
baseline validation, freezes the postcondition validation spec, and emits the
preflight / authorization / disposable-boundary receipts consumed by the
existing temp-worktree mutation chain.

It never mutates target source.
"""
...
CANDIDATE_ID = (
    "sha256:2fb4fc47394645dd1dada20e09c329ff23ae9cf1ef499176e0ba0658b04690c4"
)
...
PARSE_COMMAND = ["node", "--check", SOURCE_PATH]
TYPECHECK_COMMAND = ["yarn", "lint:typecheck"]
...
def _validate_plan_and_candidate(
    *,
    ...
    if (plan.get("source_integrity", {}) or {}).get(
        "target_source_mutated"
    ) is not False:
...
def main() -> int:
    ...
    authorization = {
        ...
        "issuer": {
            ...
            "scope_note": (
                "User-authorized disposable Prettier evidence collection only"
            ),
        },
        "binding": {
            "candidate_id": CANDIDATE_ID,
    ...
    boundary_path = out / "disposable-boundary.json"
    _write_json(boundary_path, boundary)

    summary = {
        "schema_version": "plw-js-ts-v2-prettier-sealed-preparation-v1",
        "status": "SEALED_PRETTIER_EXPERIMENT_PREPARED",
        "repository": REPOSITORY,
        "revision": revision,
        "candidate_id": CANDIDATE_ID,
        "source_path": SOURCE_PATH,
